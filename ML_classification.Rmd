...
# Find soil object by drawing polygons at least 5-6 large polygon uniformly distributed
#generate random 400 points for soil class
orto <- bandset_1
soil<-fieldView(mosaic = orto, editor = TRUE)
soil<-st_as_sf(st_sample(soil, 400)) #you can change this value
soil$class<-'soil'
...
```{r}
# ============================================================================
# HELPER FUNCTIONS - MÉTRICAS GERAIS
# ============================================================================
...
extract_metrics <- function(result, model_name) {
  ...
  if (!is.null(result$metrics)) {
    ...
    # Extrair métricas gerais (overall)
...
# ============================================================================
# HELPER FUNCTIONS - MÉTRICAS POR CLASSE
# ============================================================================
...
extract_byclass_metrics <- function(result, model_name) {
  ...
  if (is.null(byClass)) {
    warning("Métricas por classe (byClass) não encontradas para ", model_name)
    return(NULL)
  }
  ...
  if (is.matrix(byClass)) {
    ...
    warning("Formato de byClass não reconhecido para ", model_name)
    return(NULL)
  }
...
create_byclass_metric_plot <- function(byclass_df, metric_name, title = NULL) {
  ...
  if (!metric_name %in% names(byclass_df)) {
    warning("Métrica '", metric_name, "' não encontrada em byClass")
    return(NULL)
  }
  ...
  if (nrow(plot_data) == 0) {
    warning("Sem dados válidos para plotar ", metric_name)
    return(NULL)
  }
...
plot_and_save_metrics <- function(result_list,
                                   ...
                                   # Parâmetros para gráficos de métricas gerais
                                   ...
                                   # Parâmetros para gráficos de métricas por classe
  ...
  for (model_name in names(model_mapping)) {
    ...
    if (!is.null(result_list[[result_key]])) {
      ...
      if (!is.null(overall_metrics)) {
        ...
        message("  ✓ Métricas gerais extraídas")
      }
      ...
      if (!is.null(byclass_metrics)) {
        ...
        message("  ✓ Métricas por classe extraídas")
        message("    Classes encontradas: ", paste(unique(byclass_metrics$Class), collapse = ", "))
        message("    Métricas disponíveis: ", paste(setdiff(names(byclass_metrics), c("Model", "Class")), collapse = ", "))
      }
      ...
      if (include_confusion_matrix) {
        ...
        if (!is.null(conf_mat)) {
          ...
          message("  ✓ Matriz de confusão extraída")
        }
      }
      message("")
    } else {
      message("✗ Modelo ", model_name, " não encontrado\n")
    }
  }
  
  # Verificar se há dados
  if (length(available_models) == 0) {
    stop("Nenhum modelo válido encontrado.")
  }
  
  message("Total de modelos disponíveis: ", length(available_models), 
          " (", paste(available_models, collapse = ", "), ")\n")
  
  # ============================================================================
  # PARTE 1: MÉTRICAS GERAIS (OVERALL)
  # ============================================================================
  ...
  if (length(all_overall_metrics) > 0) {
    message("=== PROCESSANDO MÉTRICAS GERAIS ===\n")
    ...
    if (length(overall_metrics_to_plot) == 0) {
      warning("Nenhuma das métricas gerais solicitadas foi encontrada.\n",
              "Métricas disponíveis: ", paste(available_overall_metrics, collapse = ", "))
    } else {
      message("Métricas gerais a serem plotadas: ", paste(overall_metrics_to_plot, collapse = ", "), "\n")
      ...
      # Layout automático para métricas gerais
      ...
      # Plot combinado de métricas gerais
      if (n_metrics == 1) {
      ...
      message("\n✓ Plot combinado de métricas gerais salvo")
      message("  Layout: ", overall_metrics_nrow, "x", overall_metrics_ncol, "\n")
    }
  }
  
  # ============================================================================
  # PARTE 2: MÉTRICAS POR CLASSE
  # ============================================================================
  ...
  if (length(all_byclass_metrics) > 0) {
    message("=== PROCESSANDO MÉTRICAS POR CLASSE ===\n")
    ...
    if (length(byclass_metrics_to_plot) == 0) {
      warning("Nenhuma das métricas por classe solicitadas foi encontrada.\n",
              "Métricas disponíveis: ", paste(available_byclass_metrics, collapse = ", "))
    } else {
      message("Métricas por classe a serem plotadas: ", paste(byclass_metrics_to_plot, collapse = ", "), "\n")
      ...
      for (metric in byclass_metrics_to_plot) {
        ...
        if (!is.null(plot)) {
          ...
          ggsave(plot_path, plot, 
                 ...
                 dpi = dpi)
          message("  → ", metric, " por classe salvo")
          byclass_metric_plots[[metric]] <- plot
        }
      }
      
      # Layout automático para métricas por classe
      ...
      # Plot combinado de métricas por classe
      if (n_metrics == 1) {
      ...
      message("\n✓ Plot combinado de métricas por classe salvo")
      message("  Layout: ", byclass_metrics_nrow, "x", byclass_metrics_ncol, "\n")
    }
  }
  
  # ============================================================================
  # PARTE 3: MATRIZES DE CONFUSÃO
  # ============================================================================
  ...
  if (include_confusion_matrix && length(all_conf_matrices) > 0) {
    message("=== PROCESSANDO MATRIZES DE CONFUSÃO ===\n")
    ...
    for (model_name in names(all_conf_matrices)) {
      ...
      ggsave(cm_path, plot, 
             ...
             dpi = dpi)
      message("  → Matriz ", model_name, " salva")
      
      cm_plots[[model_name]] <- plot
    }
    
    # Layout automático para matrizes
  ...
  if (!is.null(overall_metrics_df)) {
    ...
    message("✓ Tabela de métricas gerais: overall_metrics_summary.csv")
  }
  ...
  if (!is.null(byclass_metrics_df)) {
    ...
    message("✓ Tabela de métricas por classe: byclass_metrics_summary.csv")
  }
  
  message("\n", strrep("=", 60))
  message("✅ PROCESSAMENTO CONCLUÍDO!")
  message("📁 Todos os arquivos salvos em: ", save_dir)
  message(strrep("=", 60), "\n")
...
result <- plot_and_save_metrics(
  ...
  # Métricas gerais (overall) - as mesmas de antes
  overall_metrics_to_plot = c("Accuracy", "Kappa"),
  
  # NOVO: Métricas por classe - escolha as que você quer
  byclass_metrics_to_plot = c("Sensitivity", "Specificity", "Precision", 
                              "Recall", "F1", "Balanced Accuracy"),
  
  include_confusion_matrix = TRUE,
  
  # Layout das métricas gerais
  overall_metrics_nrow = 1,
  overall_metrics_ncol = 2,
  
  # Layout das métricas por classe
  byclass_metrics_nrow = 3,
  byclass_metrics_ncol = 2,
  byclass_metrics_width = 16,
  byclass_metrics_height = 18,
  
  # Layout das matrizes de confusão
  cm_nrow = 1,
  cm_ncol = 3,
  cm_width = 18,
  cm_height = 6,
...
print("Métricas gerais disponíveis:")
